...
import Morphir.IR.Path.Fuzzer exposing (pathFuzzer, pathFuzzerOfLength)
...
isPrefixOfTests =
    ...
    describe "fromString"
        ...
        , fuzz pathFuzzer
            "path after droppinga name from the end is prefix of original"
            (\samplePath ->
                let
                    _ =
                        Debug.log "samplePath" (Path.toString Name.toTitleCase "." samplePath)
                in
                Expect.pass
            )
        ]
